...
FILE_PREF=$NAME-$DATE-test-tree-openMP
...
for N in 1048576 2097152 4194304 8388608 16777216 33554432; do \
    for thr in 2 4 8 16 32 64 128 256 512 1024 2048; do \
            echo cube N=$N && ./test_octree $N 1 128 3 18 $thr>> $FILE_PREF-cube-$N.txt ; \
    done ; \
done ;
# run octant experiments
for N in 1048576 2097152 4194304 8388608 16777216 33554432; do \
    for thr in 2 4 8 16 32 64 128 256 512 1024 2048; do \
           echo Plummer N=$N && ./test_octree $N 1 128 10 18 $thr>> $FILE_PREF-plummer-$N.txt ; \
    done ; \
done ;
